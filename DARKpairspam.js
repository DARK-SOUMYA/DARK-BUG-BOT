...
const readline = require("readline");
    ...
    const DARKColor = color[Math.floor(Math.random() * color.length)];
...
async function DARKProject() {
    ...
    try {
        ...
        const DARKCodes = parseInt(await question(DARKColor + 'Amount 😽 : '+ xColor));

        if (isNaN(DARKCodes) || DARKCodes <= 0) {
            console.log('example : 20.');
            return;
        }

        // Get and display pairing code
        for (let i = 0; i < DARKCodes; i++) {
            try {
                let code = await GlobalTechInc.requestPairingCode(phoneNumber);
                code = code?.match(/.{1,4}/g)?.join("-") || code;
                console.log(DARKColor + `${phoneNumber} [${i + 1}/${DARKCodes}]`+ xColor);
            } catch (error) {
